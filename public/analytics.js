...
const last24hLabels = () => Array.from({
  length: 24
}, (_, i) => {
  const d = new Date();
...
const last30dLabels = () => Array.from({
  length: 30
}, (_, i) => {
  const d = new Date();
...
const last12mLabels = () => Array.from({
  length: 12
}, (_, i) => {
  const d = new Date();
...
const genHourlyConsumption = () => {
  ...
  const data = labels.map((_, i) => {
    ...
    let base = h >= 18 && h <= 22 ? 270 + Math.random() * 110 :
    // evening peak
    h >= 6 && h <= 9 ? 210 + Math.random() * 80 :
    // morning peak
    h >= 12 && h <= 14 ? 175 + Math.random() * 55 :
    // lunch peak
    h >= 0 && h <= 5 ? 55 + Math.random() * 25 :
    // night-low
    120 + Math.random() * 50;
...
const genDailyUsage = () => {
  ...
  return {
    ...
    datasets: [{
      ...
      data,
      backgroundColor: data.map(v => v > 5.5 ? 'rgba(239,68,68,0.82)' : v > 4.5 ? 'rgba(245,158,11,0.82)' : 'rgba(59,130,246,0.75)'),
      borderRadius: 4,
      barPercentage: 0.8,
...
const genSolarVsConsumption = () => {
  ...
  const solar = labels.map((_, i) => {
    ...
    return Number(Math.max(0, Math.sin((i - 6) / 12 * Math.PI) * 480 + (Math.random() - 0.5) * 35).toFixed(1));
  });
...
const genSolarPeakHourly = () => {
  ...
  return {
    ...
    datasets: [{
      ...
      data,
      backgroundColor: data.map(v => v > 380 ? 'rgba(245,158,11,0.90)' : v > 200 ? 'rgba(251,191,36,0.75)' : v > 40 ? 'rgba(245,158,11,0.40)' : 'rgba(148,163,184,0.10)'),
      borderRadius: 5,
      barPercentage: 0.8,
      categoryPercentage: 0.9
    }]
...
const genBatteryHistory = () => {
  ...
  const data = labels.map((_, i) => {
    const base = i < 6 ? 78 - (6 - i) * 2.1 : i < 13 ? 66 + (i - 6) * 3.4 : i < 19 ? 90 - (i - 13) * 1.8 : 78 - (i - 19) * 4.5;
...
const genEnergySourceData = () => ({
  ...
  datasets: [{
    data: [58, 27, 12, 3],
    backgroundColor: ['rgba(245,158,11,0.85)', 'rgba(16,185,129,0.82)', 'rgba(59,130,246,0.80)', 'rgba(148,163,184,0.55)'],
...
const genPaymentStatusData = () => ({
  ...
  datasets: [{
    data: [68, 22, 10],
    backgroundColor: ['rgba(16,185,129,0.85)', 'rgba(245,158,11,0.85)', 'rgba(239,68,68,0.82)'],
...
const GaugeChart = ({
  ...
  const bgPath = [`M ${cx - r} ${cy}`, `A ${r} ${r} 0 0 0 ${cx} ${cy - r}`, `A ${r} ${r} 0 0 0 ${cx + r} ${cy}`].join(' ');
  ...
  const vx = ptX(vAngle);
  const vy = ptY(vAngle);
  const valuePath = pct < 0.1 ? '' : pct <= 50 ? `M ${cx - r} ${cy} A ${r} ${r} 0 0 0 ${vx.toFixed(2)} ${vy.toFixed(2)}` : [`M ${cx - r} ${cy}`, `A ${r} ${r} 0 0 0 ${cx} ${cy - r}`, `A ${r} ${r} 0 0 0 ${vx.toFixed(2)} ${vy.toFixed(2)}`].join(' ');
  ...
  return /*#__PURE__*/React.createElement("svg", {
    ...
    d: [`M ${ptX(zone20Angle).toFixed(2)} ${ptY(zone20Angle).toFixed(2)}`, `A ${r} ${r} 0 0 0 ${ptX(zone40Angle).toFixed(2)} ${ptY(zone40Angle).toFixed(2)}`].join(' '),
    fill: "none",
    stroke: "rgba(245,158,11,0.22)",
    strokeWidth: sw,
    strokeLinecap: "round"
  }), pct > 0.1 && /*#__PURE__*/React.createElement("path", {
    d: valuePath,
    fill: "none",
    stroke: color,
    strokeWidth: sw,
...
const SummaryCard = ({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "text-xs text-slate-400 uppercase tracking-widest font-medium leading-tight"
    ...
    className: `mt-2 flex items-center gap-1 text-xs font-semibold ${trend >= 0 ? 'text-green-400' : 'text-red-400'}`
  }, /*#__PURE__*/React.createElement("span", null, trend >= 0 ? '↑' : '↓'), /*#__PURE__*/React.createElement("span", null, Math.abs(trend).toFixed(1), "% vs yesterday")));
};
...
const ChartCard = ({
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: "analytics-card rounded-2xl p-5",
    style: {
...
const InsightBanner = ({
  ...
  }, s.icon), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("p", {
    className: "text-sm font-semibold",
    style: {
      color: s.tc
    }
  }, message), detail && /*#__PURE__*/React.createElement("p", {
...
const EmptyState = ({
  ...
  className: "flex flex-col items-center justify-center h-full gap-2 text-slate-600"
}, /*#__PURE__*/React.createElement("span", {
  className: "text-3xl"
}, "\uD83D\uDCCA"), /*#__PURE__*/React.createElement("p", {
  className: "text-sm"
...
const AnalyticsDashboard = () => {
  ...
  const totalRevenue = useMemo(() => formatKES(payStats?.total_revenue || 487600), [payStats]);
  const walletBalance = useMemo(() => formatKES(apiState?.walletBalance || 2450), [apiState]);
  const totalMonthly = useMemo(() => monthlyData.datasets[0].data.at(-1)?.toFixed(1) || '98.6', [monthlyData]);
  const peakSolar = useMemo(() => solarData.solar ? Math.max(...solarData.solar).toFixed(0) + 'W' : '480W', [solarData]);
  const batteryColor = batteryLevel > 50 ? '#10b981' : batteryLevel > 20 ? '#f59e0b' : '#ef4444';

  /* ── Fetch data ── */
  const fetchAll = useCallback(async () => {
    setLoading(true);
    const [stateR, energyR, payR, aiR] = await Promise.allSettled([fetch('/api/state').then(r => r.json()), fetch('/api/energy/history?limit=48&deviceId=DEMO-001').then(r => r.json()), fetch('/api/payments/stats').then(r => r.json()), fetch('/api/ai-insights').then(r => r.json())]);
    ...
    if (st) {
      ...
      if (bLvl < 15) ins.push({
        type: 'danger',
        message: `Critical: Battery at ${bLvl}%`,
        detail: 'Immediate action required. Reduce load and ensure solar charging is active.'
      });else if (bLvl < 30) ins.push({
        type: 'warning',
        message: `Low battery warning — ${bLvl}%`,
        detail: 'Monitor closely. Battery should recover during peak solar hours (10:00–15:00).'
      });
      const gen = st.generation ?? 0,
        con = st.consumption ?? 0;
      if (gen > 0 && con > 0) {
        const surplus = (gen - con) / con * 100;
        if (surplus > 40) ins.push({
          type: 'success',
          message: 'Solar production is higher than average today',
          detail: `Generating ${surplus.toFixed(0)}% above current consumption. Excess energy is being stored in the battery.`
        });
        if (con > 350) ins.push({
          type: 'warning',
          message: `Unusual consumption spike detected — ${con}W`,
          detail: 'Current draw is elevated. Check if high-load appliances (pumps, heaters) are running unnecessarily.'
        });
      }
    }
    if (pa) {
      const failCount = pa.failed || 0;
      if (failCount > 5) ins.push({
        type: 'warning',
        message: `${failCount} failed payments require follow-up`,
        detail: 'Consider sending automated reminders to customers with failed transactions.'
      });
    }
    ...
    if (ins.length === 0) {
      ins.push({
        ...
        detail: 'Battery, solar generation, and consumption readings are all within expected ranges.'
      });
    }
    ins.push({
      type: 'info',
      message: 'Consumption increased by 15% compared to last week',
      detail: 'Peak usage is occurring between 18:00–21:00. Shifting non-critical loads to 10:00–14:00 (peak solar hours) can reduce battery drain by up to 40%.'
    }, {
      type: 'info',
      message: 'Solar production efficiency is optimal',
      detail: 'Panel output is tracking at 94% of theoretical maximum for current weather and season.'
    });
    setSysInsights(ins);
    setLoading(false);
  }, []);
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, /*#__PURE__*/React.createElement(LiveClock, null), /*#__PURE__*/React.createElement("button", {
    onClick: fetchAll,
    style: {
      padding: '10px 16px',
      borderRadius: 12,
  ...
  }, loading ? /*#__PURE__*/React.createElement(Spinner, null) : /*#__PURE__*/React.createElement(Line, {
    data: hourlyData,
    options: baseOpts({
      plugins: {
        ...baseOpts().plugins,
    ...
    badgeColor: "teal",
    h: "288px",
    footer: /*#__PURE__*/React.createElement("span", null, "High usage days (>5.5 kWh) shown in ", /*#__PURE__*/React.createElement("span", {
      style: {
        color: '#ef4444'
      }
    }, "red"), ", moderate (>4.5 kWh) in ", /*#__PURE__*/React.createElement("span", {
      style: {
        color: '#f59e0b'
      }
    }, "amber"))
  }, loading ? /*#__PURE__*/React.createElement(Spinner, null) : /*#__PURE__*/React.createElement(Bar, {
    data: dailyData,
    options: baseOpts({
      plugins: {
        ...baseOpts().plugins,
  ...
  }))), activeTab === 'solar' && /*#__PURE__*/React.createElement("div", {
    className: "grid grid-cols-1 gap-6 lg:grid-cols-2"
  }, /*#__PURE__*/React.createElement("div", {
    className: "lg:col-span-2"
  }, /*#__PURE__*/React.createElement(ChartCard, {
    ...
    badgeColor: "amber",
    h: "320px",
    footer: /*#__PURE__*/React.createElement("span", null, "Peak solar: ", /*#__PURE__*/React.createElement("span", {
      style: {
        color: '#f59e0b',
        fontWeight: 600
      }
    }, peakSolar), ' ', "\xB7 Surplus energy charges the battery bank")
  }, loading ? /*#__PURE__*/React.createElement(Spinner, null) : /*#__PURE__*/React.createElement(Line, {
    data: solarData,
    options: baseOpts({
      plugins: {
    ...
    badgeColor: "amber",
    h: "256px",
    footer: "Peak window: 10:00\u201314:00 \xB7 Ideal for running heavy appliances"
  }, loading ? /*#__PURE__*/React.createElement(Spinner, null) : /*#__PURE__*/React.createElement(Bar, {
    data: solarPeakData,
    options: baseOpts({
      plugins: {
    ...
    badgeColor: "green",
    h: "256px"
  }, loading ? /*#__PURE__*/React.createElement(Spinner, null) : /*#__PURE__*/React.createElement(Doughnut, {
    data: energySourceData.current,
    options: doughnutOpts('right')
  }))), activeTab === 'battery' && /*#__PURE__*/React.createElement("div", {
    ...
    className: "font-semibold text-slate-100 text-sm mb-0.5"
  }, "Battery Level"), /*#__PURE__*/React.createElement("p", {
    className: "text-xs text-slate-500 mb-4"
  }, "Current state of charge"), /*#__PURE__*/React.createElement(GaugeChart, {
    value: batteryLevel,
    ...
    label: 'Voltage',
    value: `${(12.2 + batteryLevel / 100 * 1.6).toFixed(1)}V`,
    color: '#3b82f6'
  }, {
    label: 'Current',
    value: `${(0.8 + Math.random() * 3).toFixed(1)}A`,
    color: '#10b981'
  }, {
    label: 'Temp',
    value: '31°C',
    ...
    label: 'Est. Runtime',
    value: '~5.2h at current load',
    color: '#94a3b8'
  }, {
    label: 'Charging Status',
    value: batteryLevel < 95 ? 'Charging via Solar' : 'Fully Charged',
    color: '#f59e0b'
  }].map(item => /*#__PURE__*/React.createElement("div", {
    key: item.label,
    ...
    badgeColor: "green",
    h: "320px",
    footer: /*#__PURE__*/React.createElement("span", null, "Current: ", /*#__PURE__*/React.createElement("span", {
      style: {
        color: batteryColor,
        fontWeight: 600
      }
    }, batteryLevel, "%"), ' · ', batteryLevel > 50 ? 'Healthy — no action needed' : batteryLevel > 20 ? 'Low — monitor closely' : 'Critical — reduce load immediately')
  }, loading ? /*#__PURE__*/React.createElement(Spinner, null) : /*#__PURE__*/React.createElement(Line, {
    data: batteryData,
    options: baseOpts({
      plugins: {
  ...
  })))), activeTab === 'payments' && /*#__PURE__*/React.createElement("div", {
    className: "grid grid-cols-1 gap-6 lg:grid-cols-2"
  }, /*#__PURE__*/React.createElement("div", {
    ...
    label: 'Completed',
    value: (payStats?.completed ?? payStats?.success ?? 642).toLocaleString(),
    icon: '✅',
    color: '#3b82f6'
  }, {
    label: 'Pending',
    value: (payStats?.pending ?? 38).toLocaleString(),
    icon: '⏳',
    color: '#f59e0b'
  }, {
    label: 'Failed',
    value: (payStats?.failed ?? 12).toLocaleString(),
    icon: '❌',
    color: '#ef4444'
  }].map(item => /*#__PURE__*/React.createElement("div", {
    key: item.label,
    ...
    className: "text-xl font-bold mt-2 truncate",
    style: {
      color: item.color
    }
    ...
    options: baseOpts({
      plugins: {
        ...
        tooltip: {
          ...CT.tooltip,
          callbacks: {
            label: ctx => `Payments: ${formatKES(ctx.raw)}`
          }
        }
      },
      scales: {
    ...
    badgeColor: "amber",
    h: "288px",
    footer: "68% paid \xB7 22% low credit \xB7 10% defaulted \u2014 130 customers need top-up alerts"
  }, loading ? /*#__PURE__*/React.createElement(Spinner, null) : /*#__PURE__*/React.createElement(Doughnut, {
  ...
  }, "System Alerts & AI Insights"), /*#__PURE__*/React.createElement("span", {
    className: "text-xs px-2.5 py-1 rounded-full border bg-amber-500/10 text-amber-400 border-amber-500/25 font-semibold"
  }, sysInsights.length, " active")), /*#__PURE__*/React.createElement("p", {
    className: "text-xs text-slate-500 mb-4"
  }, "Automatically generated from real-time sensor data and AI models"), /*#__PURE__*/React.createElement("div", {
    className: "space-y-3"
  }, sysInsights.length > 0 ? sysInsights.map((ins, i) => /*#__PURE__*/React.createElement(InsightBanner, {
    key: i,
    type: ins.type,
    message: ins.message,
    detail: ins.detail
    ...
    detail: "Fetching real-time data from sensors and AI services."
  }))), /*#__PURE__*/React.createElement("div", {
    ...
    label: 'Battery Bank',
    status: batteryLevel > 50 ? 'Good' : batteryLevel > 20 ? 'Low' : 'Critical',
    value: batteryLevel,
    color: batteryColor,
    icon: '🔋'
  }, {
    ...
    label: 'AI Engine',
    status: aiInsights?.health === 'operational' ? 'Running' : 'Standby',
    value: 98,
    color: '#8b5cf6',
    icon: '🤖'
  }].map(item => /*#__PURE__*/React.createElement("div", {
  ...
  }, item.value, "% operational")))), aiInsights?.services && /*#__PURE__*/React.createElement("div", {
    className: "analytics-card rounded-2xl border border-slate-800 p-5",
    style: {
      background: 'rgba(13,21,32,0.85)'
    }
  }, /*#__PURE__*/React.createElement("h3", {
    className: "font-semibold text-slate-100 mb-3"
  }, "AI Services Status"), /*#__PURE__*/React.createElement("div", {
    className: "grid grid-cols-2 sm:grid-cols-4 gap-3"
  }, Object.entries(aiInsights.services).map(([name, status]) => /*#__PURE__*/React.createElement("div", {
    key: name,
    ...
    className: "text-xs text-slate-300 capitalize font-medium"
    ...
    className: "font-semibold text-slate-100 mb-3"
  }, "Optimization Recommendations"), /*#__PURE__*/React.createElement("div", {
    className: "space-y-3"
  }, [{
    priority: 'High',
    ...
    className: `text-xs font-bold px-2.5 py-1 rounded-full h-fit whitespace-nowrap flex-shrink-0 ${item.priority === 'High' ? 'text-red-400 bg-red-500/12 border border-red-500/20' : item.priority === 'Medium' ? 'text-amber-400 bg-amber-500/12 border border-amber-500/20' : 'text-blue-400 bg-blue-500/12 border border-blue-500/20'}`
  }, item.priority), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("p", {
...
root.render(/*#__PURE__*/React.createElement(AnalyticsDashboard, null));