...
const SolarDashboard = () => {
  ...
  function appendLivePoint(d) {
    const label = new Date(d.ts || Date.now()).toLocaleTimeString('en-KE', { hour: '2-digit', minute: '2-digit', second: '2-digit' });
    const append = (key, ref, values) => {
      ...
      while (series.labels.length > MAX) {
        series.labels.shift();
        series.datasets.forEach(ds => { if (Array.isArray(ds.data)) ds.data.shift(); });
      }
      chart.update('none');
    };
    ...
    if (d.voltage != null || d.current_amps != null) {
      append('voltageCurrent', voltageCurrentData, [
        d.voltage != null ? Math.round(Number(d.voltage) * 10) / 10 : null,
        d.current_amps != null ? Math.round(Number(d.current_amps) * 10) / 10 : null
      ]);
    }
  }
  ...
  return (
    <div className="min-h-screen bg-transparent text-slate-900 dark:text-slate-100 dashboard-shell">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* ── Live event ticker (relay toggles, OTA outcomes, alerts) ── */}
        {liveEvents.length > 0 && (
          <div className="mt-4 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-sm">
            <div className="flex items-center gap-2 mb-2">
              <span className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" />
              <span className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Live Events</span>
            </div>
            <div className="flex flex-wrap gap-2">
              {liveEvents.map((ev, i) => (
                <span key={i} className="text-xs rounded-lg px-2.5 py-1 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300">
                  {ev.kind === 'relay' ? `${ev.deviceId} relay → ${ev.state}` :
                    ev.kind === 'ota' ? `${ev.deviceId} OTA ${ev.status} · ${ev.version}` :
                      ev.kind === 'alert' ? `${ev.deviceId || ''} ${ev.severity}: ${ev.message}` : ev.type}
                </span>
              ))}
            </div>
          </div>
