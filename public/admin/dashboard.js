...
const buildBatteryStateData = (labels, batteryPct) => {
  const pointColors = batteryPct.map(value => value > 50 ? 'rgb(34, 197, 94)' : value > 20 ? 'rgb(245, 158, 11)' : 'rgb(239, 68, 68)');
  const background = batteryPct.map(value => value > 50 ? 'rgba(34, 197, 94, 0.24)' : value > 20 ? 'rgba(245, 158, 11, 0.24)' : 'rgba(239, 68, 68, 0.24)');
  return {
    labels,
    datasets: [{
      label: 'Battery Level (%)',
...
const fetchForecast = async (deviceId = 'DEMO-001') => {
  try {
    ...
    const res = await fetch(`/api/forecast?deviceId=${deviceId}`, {
      headers
    });
...
const fetchMaintenanceAlerts = async (deviceId = 'DEMO-001') => {
  try {
    ...
    const res = await fetch(`/api/maintenance-alerts?deviceId=${deviceId}`, {
      headers
    });
    if (!res.ok) return null;
    const data = await res.json();
...
const fetchAppliancePerformance = async (deviceId = 'DEMO-001') => {
  try {
    ...
    const res = await fetch(`/api/appliances?deviceId=${encodeURIComponent(deviceId)}`, {
      headers
    });
...
const SolarDashboard = () => {
  ...
  const applianceShareData = useRef({
    ...buildApplianceShareData([]),
    avgConsumption: null
  });
  ...
  const refreshAllCharts = useCallback(async () => {
    ...
    const [appliancePerf, forecast, maintenanceAlerts, devices] = await Promise.all([fetchAppliancePerformance(selectedDeviceId), fetchForecast(selectedDeviceId), fetchMaintenanceAlerts(selectedDeviceId), fetchDevices()]);
    if (appliancePerf) {
      ...
      const newApplianceShare = {
        ...buildApplianceShareData(appliances),
        avgConsumption: appliancePerf.avgConsumption ?? null
      };
    ...
    if (devices && devices.length > 0) {
      ...
      for (const d of devices) {
        ...
        if (!d.online) regionMap[region].offline++;else if (latestBatt < 20) regionMap[region].lowBattery++;else regionMap[region].online++;
      }
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        let res = await fetch('/api/admin/devices', {
          headers
        });
        let devices = [];
        if (res.ok) {
  ...
  const applianceLegend = applianceShareData.current.labels.map((label, i) => {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: `w-2.5 h-2.5 rounded-full ${heavy ? 'bg-rose-500' : applianceShareDotColors[i % applianceShareDotColors.length]}`
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, orgInfo.name), "'s devices, appliances & analytics")), /*#__PURE__*/React.createElement("div", {
    ...
    title: 'Appliances Tracked',
    value: appliancesTracked.toLocaleString(),
    accent: 'text-blue-600 dark:text-blue-400'
  }, {
    title: 'Heavy-Load Alerts',
    value: heavyLoadCount.toLocaleString(),
  ...
  }))), activeTab === 'appliances' && /*#__PURE__*/React.createElement("div", {
    className: "mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2"
  }, /*#__PURE__*/React.createElement(ChartCard, {
    title: "Appliance Load Breakdown",
    badge: `${appliancesTracked} Tracked`,
    badgeColor: "emerald",
    subtitle: "Effective wattage per registered appliance \u2014 red bars are flagged heavy.",
    footer: applianceShareData.current.avgConsumption ? /*#__PURE__*/React.createElement(React.Fragment, null, "Typical total load: ", /*#__PURE__*/React.createElement("span", {
      className: "font-semibold text-amber-600 dark:text-amber-400"
    }, Math.round(applianceShareData.current.avgConsumption), "W"), " (last 7 days)") : 'Not enough history yet to estimate typical load.'
  }, applianceLoadData.current.labels.length > 0 ? /*#__PURE__*/React.createElement(Bar, {
    ref: mkRef('applianceLoad'),
    data: applianceLoadData.current,
    options: {
      ...baseOptions,
      plugins: {
        ...baseOptions.plugins,
  ...
  }, "No appliances registered yet \u2014 add one from the admin dashboard.")), /*#__PURE__*/React.createElement(ChartCard, {
    title: "Load Share by Appliance",
    badge: "Snapshot",
    badgeColor: "amber",
    subtitle: "Each appliance's share of the estimated total load.",
    h: "320px",
    ...
    }, heavyLoadCount), " appliance(s) flagged heavy \u2014 driving the bill")
  }, applianceShareData.current.labels.length > 0 ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    style: {
      height: '200px'
    }
  }, /*#__PURE__*/React.createElement(Doughnut, {
    ref: mkRef('applianceShare'),
    data: applianceShareData.current,
    options: {
      ...baseOptions,
      plugins: {
        ...baseOptions.plugins,
  ...
  }, applianceLegend)) : /*#__PURE__*/React.createElement("div", {
    className: "flex items-center justify-center h-48 text-sm text-slate-400"
  }, "No appliances registered yet."))), activeTab === 'ai' && /*#__PURE__*/React.createElement("div", {
    className: "mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2"
  }, /*#__PURE__*/React.createElement(ChartCard, {
  ...
  }))), activeTab === 'operations' && /*#__PURE__*/React.createElement("div", {
    className: "mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2"
  }, /*#__PURE__*/React.createElement(ChartCard, {
