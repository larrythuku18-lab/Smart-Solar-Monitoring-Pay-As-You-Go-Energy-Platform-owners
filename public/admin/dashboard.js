...
const fetchFraudRiskData = async () => {
  try {
    ...
    const res = await fetch('/api/admin/fraud-risk', {
      headers
    });
    if (!res.ok) return null;
    const data = await res.json();
...
const fetchCreditScoreTrendData = async () => {
  try {
    ...
    const res = await fetch('/api/customers/credit-score-trend', {
      headers
    });
    return res.ok ? res.json() : null;
  } catch (err) {
...
const SolarDashboard = () => {
  ...
  const refreshAllCharts = useCallback(async () => {
    ...
    if (paymentTrend && paymentTrend.length > 0) {
      ...
      for (let i = 0; i < 12; i++) {
        ...
        if (idx >= 0) mrrValues.push(paymentTrend[idx].revenue);else mrrValues.push(0);
      }
      mrrValues.reverse();
      const mrrLabels = Array.from({
        length: 12
      }, (_, i) => {
        const date = new Date();
        date.setDate(1);
  ...
  const creditLegend = creditScoreTrendData.current.datasets.map((ds, i) => /*#__PURE__*/React.createElement("div", {
    key: ds.label,
    className: "flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300"
  }, /*#__PURE__*/React.createElement("span", {
    className: `w-2.5 h-2.5 rounded-full ${creditScoreDotColors[i % creditScoreDotColors.length]}`
  ...
  }, creditScoreTrendData.current.datasets.length > 0 ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(Line, {
  ...
  }, creditLegend)) : /*#__PURE__*/React.createElement("div", {
    className: "flex items-center justify-center h-48 text-sm text-slate-400"
  }, "Not enough customer payment history yet."))), activeTab === 'operations' && /*#__PURE__*/React.createElement("div", {
    className: "mt-6 grid grid-cols-1 gap-6 lg:grid-cols-2"
  }, /*#__PURE__*/React.createElement(ChartCard, {
