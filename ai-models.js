 ...
 * Energy forecasting, predictive maintenance, fraud detection, optimization
 */
...
const FALLBACK_FORECAST = Array.from({ length: 6 }, (_, i) => ({
  hour: i + 1,
  predictedGeneration: 150,
  predictedConsumption: 120,
  surplus: 30,
  confidence: 0.5
}));
...
const FALLBACK_OPTIMIZATION = [{
  type: 'payment_timing',
  priority: 'medium',
  message: 'Top up during midday for best solar charging',
  optimalHour: 3
}];

function safeForecast(horizonHours = 6) {
  try {
    if (!forecaster.ready && historyData.generation.length < 3) {
...
function seedFromEnergyReadings(readings) {
  try {
    for (const r of readings) {
      forecaster.addDataPoint(r.generation_watts, r.consumption_watts, Date.now());
    ...
    console.warn('AI seed warning:', err.message);
  }
}
